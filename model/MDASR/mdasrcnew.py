...
class FIFM(nn.Module):#cross-fussion moudle
    def __init__(self, in_channels, out_channels, scale=2):
        ...
        self.conv2=B.conv_layer( in_channels, in_channels, kernel_size=1)
...
class MDASR(nn.Module):
    def __init__(self, in_channels=3 ,out_channels=60,num_modules=6,scale=2):
    ...
    def forward(self,input):
        ...
        out_c,out_s=self.FIFM(out_pixel)
        out_B1 = self.CDM1(out_c)
        ...
        out_lr = self.esa(out_B) + out_c
        output = self.upsampler(out_lr)
        output = self.conv(output+out_s)
        return output